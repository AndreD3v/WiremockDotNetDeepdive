    ...
    public class Answers04 : TestBase
    {
        public void SetupStubExercise401()
        {
            ...
            .RespondWith(
                Response.Create()
                    .WithStatusCode(200)
                    .WithBody("Listening on port {{request.port}}").WithTransformer()
            );
        }
        ...
        public void SetupStubExercise402()
        {
            ...
            server.Given(
                Request.Create()
                    .WithPath("/echo-loan-amount")
                .UsingPost()
            )
            .RespondWith(
                Response.Create()
                    .WithStatusCode(201)
                    .WithBody("Received loan application request for ${{JsonPath.SelectToken request.body \"$.loanDetails.amount\"}}").WithTransformer()
            );
        }
